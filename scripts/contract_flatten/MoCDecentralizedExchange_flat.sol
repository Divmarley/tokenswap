...
@param _secondaryToken Address of the secondary token of the pair
*/
  function disableTokenPair(address _baseToken, address _secondaryToken) public virtual  onlyAuthorizedChanger {
...
@param _secondaryToken Address of the secondary token of the pair
*/
  function enableTokenPair(address _baseToken, address _secondaryToken) public virtual onlyAuthorizedChanger {
    MoCExchangeLib.Pair storage pair = getTokenPair(_baseToken, _secondaryToken);
...
*/
  ...
  ) public virtual onlyAuthorizedChanger() isNewPairValid(_baseToken, _secondaryToken) {
...
whenNotPaused {
    ...
    //   nextId(),
    //   SafeMath.sub(_amount, initialFee),
    //   initialFee,
    //   _multiplyFactor,
    //   _lifespan,
...
// File: contracts/PartialExecutionLibrary.sol
...
contract MoCDecentralizedExchange is
    EventfulExchange,
    RestrictiveOrderListing,
    PartialExecution
{
    using SafeMath for uint256;
    using MoCExchangeLib for MoCExchangeLib.Pair;
  ...
  */
    function matchOrdersWithHints(
        ...
        MoCExchangeLib.TickPaginationMemory storage pageMemory = getTokenPair(
            _baseToken,
            _secondaryToken
        ).pageMemory;
    ...
    function processExpired(
        ...
        MoCExchangeLib.Pair storage pair = getTokenPair(
            _baseToken,
            _secondaryToken
        );
        MoCExchangeLib.processExpired(
            pair,
            commissionManager,
            _evaluateBuyOrders,
            _orderId,
  ...
  */
        ...
        )
    {
        ...
        (
            ...
            lastBuyMatchAmount,
            lastSellMatchId
        ) = getEmergentPrice(_baseToken, _secondaryToken);
  ...
  */
    function getLastClosingPrice(
        address _baseToken,
        address _secondaryToken
    ) external view returns (uint256 lastClosingPrice) {
  ...
  */
    function cancelSellOrder(
        ...
        uint256 _orderId,
        uint256 _previousOrderIdHint
    ) public override whenTickIsNotRunning(_baseToken, _secondaryToken) {
        OrderListing.cancelSellOrder(
            _baseToken,
            _secondaryToken,
            _orderId,
            _previousOrderIdHint
  ...
  */
  ...
  @param _secondaryToken Address of the secondary token of the pair
  */
    function tickIsRunning(
        address _baseToken,
        address _secondaryToken
    ) public view returns (bool) {
        return
            getTickStage(_baseToken, _secondaryToken) !=
            MoCExchangeLib.TickStage.RECEIVING_ORDERS;
    }

    function getEmergentPrice(
        address _baseToken,
        address _secondaryToken
    )
        public
    ...
    {
        MoCExchangeLib.Pair storage pair = tokenPair(
            _baseToken,
            _secondaryToken
        );
    ...
    */
    ...
    function getTickStage(
        address _baseToken,
        address _secondaryToken
    ) public view returns (MoCExchangeLib.TickStage stage) {
  ...
  */
    function addTokenPair(
        ...
        // authorized changer
        TokenPairConverter.addTokenPair(
            _baseToken,
            _secondaryToken,
            _priceProvider,
            _priceComparisonPrecision,
        ...
        taskList[uint256(TaskTypes.SIMULATION)] = getTaskId(
            groupId,
            TaskTypes.SIMULATION
        );
        taskList[uint256(TaskTypes.MATCHING)] = getTaskId(
            groupId,
            TaskTypes.MATCHING
        );
        taskList[uint256(TaskTypes.MOVING_OUT_PENDINGS)] = getTaskId(
            groupId,
            TaskTypes.MOVING_OUT_PENDINGS
        );
        createTask(
            taskList[uint256(TaskTypes.SIMULATION)],
            simulationStepFunction,
            onSimulationStart,
            onSimulationFinish
        );
        createTask(
            taskList[uint256(TaskTypes.MATCHING)],
            matchOrdersStepFunction,
            onMatchOrdersStart,
            nullHookForTask
        );
        createTask(
            taskList[uint256(TaskTypes.MOVING_OUT_PENDINGS)],
            movePendingOrdersStepFunction,
            onMovePendingsStart,
            nullHookForTask
        );
  ...
  */
  ...
  for the execution of a tick of a given pair
  */
    function onTickStart(bytes32 _groupId) private view {
        MoCExchangeLib.Pair storage pair = getTokenPair(_groupId);
        assert(pair.tickStage == MoCExchangeLib.TickStage.RECEIVING_ORDERS);

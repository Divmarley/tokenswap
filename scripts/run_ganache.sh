...
start_ganache() {
  ...
  local accounts=(
    ...
    --account="0x4acf63dfb862d97ac1a1518483447c6f40ef0a10d978b812a8a31277551d6cc7,100000000000000000000000000"
    ...
    --account="0xe27bea17bfb8f9309733f8996b4990af06c3526fd54f3eec398ec80677f71165,100000000000000000000000000"
  )
  ...
  if [ "$TESTING" = true ]; then
    node_modules/.bin/ganache-cli --gasLimit 0xfffffffffff --port "$GANACHE_PORT" -i  --accounts 10 --defaultBalanceEther 100000000000000000
  else
    node_modules/.bin/ganache-cli --db "$NODO_DIRECTORY" --gasLimit 0xfffffffffff "${accounts[@]}" -i  --port $GANACHE_PORT
  fi

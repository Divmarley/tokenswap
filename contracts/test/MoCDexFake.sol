...
contract MoCDexFake is MoCDecentralizedExchange  {
  ...
  function editOrder(
    ...
    MoCExchangeLib.Order storage order = orderbook.orders[_orderId]; // Assuming the mapping inside orderbook is named 'orders'
  ...
  function iterateOrdersFromFirstOrder(
    ...
    uint256 _index
  ) internal pure returns (MoCExchangeLib.Order memory) {
